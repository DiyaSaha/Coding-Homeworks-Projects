...
for position in range(len(genome))
      for individual_seq in sequence list
            if individual_seq[position] != ref[position]
                mykey = # reflect allele and position
                if mykey in dict_SNP
                     dict_SNP[mykey]= dict_SNP[mykey] + 1
                else 
                    add mykey to the dict_SNP
'''
dict_SNP = {}
gene_ref = seq[0]
...
# for u in keys_lst: 
#     pos_in_seq, alt_SNP, ref_SNP = u.split(';')
#     SNP_freq = dict_SNP[u]/len(seq)
#     print(pos_in_seq,'\t',alt_SNP,'\t',ref_SNP,'\t',SNP_freq, file = output2)
